...
def explorationOrExplotation(learning_parameters, steps_done):
    draw = random.random()
    eps_threshold = learning_parameters.EPS_END \
        + (learning_parameters.EPS_START - learning_parameters.EPS_END) \
            * math.exp(-1 * steps_done/learning_parameters.EPS_DECAY)
...
class DeepBoardLearner(Learner):
    ...
    def choseAction(self, state, nb_action, steps_done):
        if explorationOrExplotation(self.learning_parameters, steps_done):
            with torch.no_grad():
                return self.policy_net(state).max(0)[1].view(1)
    ...
    def train(self, batch):
        ...
        next_state_max_q_values = self.target_net(next_states).max(1)[0].detach()

        bellman_expected_q_value = (next_state_max_q_values * self.learning_parameters.GAMMA) + reward_batch
